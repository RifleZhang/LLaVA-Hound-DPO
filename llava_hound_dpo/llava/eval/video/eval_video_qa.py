...
client = openai.AzureOpenAI(
    azure_endpoint="https://search-sg.bytedance.net/gpt/openapi/online/v2/crawl",
    api_version="2023-05-15",
    api_key="nlCj9fvmLyJIYBMhB9jmFgStVaT8XWEM",
)

def openai_generate(model_name, prompt, **kwargs):
    messages = []
...
def annotate(prediction_set, caption_files, output_dir, args):
    ...
    for file in caption_files:
        ...
        try:
            ...
            prompt = prompt_template.format(question=question, answer=answer, prediction=pred)

            system_prompt = format_docstring(SYSTEM_PROMPT_TEMPLATE)
            # Compute the correctness score
            response_message = openai_generate("gpt-3.5-turbo", prompt=prompt, system_prompt=system_prompt)
...
def main():
    ...
    # print("new_pred_contents: ", len(new_pred_contents))
    # exit()
    if args.num_samples:
        new_pred_contents = new_pred_contents[:args.num_samples]
    '''
    ...
    while True:
        try:
            ...
            n_completed_files = len([f for f in completed_files if f != "res.json" and f != "results.json"])
            print(f"completed_files: {n_completed_files}")
            ...
            # annotate(prediction_set, incomplete_files, output_dir, args)
    ...
    res_path = os.path.join(args.output_dir, "res.json")

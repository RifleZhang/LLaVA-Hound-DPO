...
def load_frames(frames_dir):
    results = []
    for frame_name in [x for x in os.listdir(frames_dir) if x.endswith('jpg')]:
...
def run_inference(args):
    ...
    def model_infer(question, images_tensor):
        # Question input here
        qs = question + " Please answer yes or no."
    ...
    for i, data in tqdm(enumerate(valid_data)):
        ...
        for qa in data['qas']:
            ...
            pred = model_infer(qs, images_tensor)
            qa['pred'] = pred

            # print(f"q: {qs}, a: {gold_ans}, pred: {pred}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_path", type=str,
                        default="/mnt/bn/algo-masp-nas-2/baiyi.by/checkpoints/videollava-videoproj-pretrain-frames-base-intvid-base-lr-ft-0913/")

...
class CHAIR(object):
    def __init__(self):
        ...
        with open('/mnt/bd/bohanzhaiv0/LLM/POPE/chair/cap2obj_prompt.txt', 'r') as file:
            content = file.read()
        ...
        with open('/mnt/bd/bohanzhaiv0/LLM/POPE/chair/coverage_prompt.txt', 'r') as file:
            content = file.read()
...
# user_prompt = f"I have an object list {objects_in_image} we call it list A, and a model captured an object list {obj_list} we call it list B, if list B's object or similar meaning but not same word occurred in list A, we called it a match, if either same words, synonyms or similar meaning object not occurred in list A we call it mismatch, plz return a list of mismatch. the output should be a list like mismatch = ['human', 'beach'], I don't a code return your answer"
    ...
    def compute_chair_video(self, eval_file):
...
if __name__ == '__main__':
    ...
    evaluator = CHAIR()
    cap_dict = evaluator.compute_chair_video('/mnt/bn/algo-masp-nas-2/baiyi.by/checkpoints/videollava-videoproj-rerun-pretrain-base-m3it-ft-1006/video_chair/vid_top1k_res.json')
    ...
    # elif 
    # else:
    #     uncover_dict = evaluator.converage(args.cap_file, '/mnt/bn/data-tns-algo-masp/data/VisualGenome_task')
    #     print(uncover_dict)
...
class LlavaMetaModel:
    ...
    def __init__(self, config):
        ...
        import os
        cache_dir = os.environ['cache_dir'] if 'cache_dir' in os.environ else './cache'
        
        if hasattr(config, "mm_image_tower"):
            self.image_tower = build_image_tower(config, delay_load=True, cache_dir=cache_dir)
            self.mm_projector = build_vision_projector(config)
        if hasattr(config, "mm_video_tower"):
            self.video_tower = build_video_tower(config, delay_load=True, cache_dir=cache_dir)
            self.mm_projector = build_vision_projector(config)
        ...
        if model_args.mm_use_x_start_end:
            ...
            if model_args.pretrain_mm_mlp_adapter:
                ...
                if 'model.embed_tokens.weight' in mm_projector_weights:
                    ...
                    print(f"Pretrained mm_mlp_adapter does not have 'model.embed_tokens.weight'. Skip loading.")
